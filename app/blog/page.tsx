...
const postsQuery = defineQuery(/* groq */ `*[_type == "post" && defined(slug.current)] | order(publishedAt desc, _id asc){_id,title,"slug":slug.current,excerpt,publishedAt}`)

export default async function BlogPage() {
  let posts: Post[] = []
  if (process.env.NEXT_PUBLIC_SANITY_PROJECT_ID) { const { client } = await import('@/lib/sanity/client'); posts = await client.fetch<Post[]>(postsQuery).catch(() => []) }
